...
require dirname(__FILE__) . '/src/XdbSearcher.php';
...
switch ( $cachePolicy ) {
    case 'file':
        try {
            $searcher = yzh52521\XdbSearcher::newWithFileOnly($dbFile);
        } catch (Exception $e) {
        ...
        break;
    case 'vectorIndex':
        $vIndex = yzh52521\XdbSearcher::loadVectorIndexFromFile($dbFile);
        if ($vIndex == null) {
        ...
        try {
            $searcher = yzh52521\XdbSearcher::newWithVectorIndex($dbFile, $vIndex);
        } catch (Exception $e) {
        ...
        break;
    case 'content':
        $cBuff = yzh52521\XdbSearcher::loadContentFromFile($dbFile);
        if ($cBuff == null) {
        ...
        try {
            $searcher = yzh52521\XdbSearcher::newWithBuffer($cBuff);
        } catch (Exception $e) {
...
while ( true ) {
    ...
    if (yzh52521\XdbSearcher::ip2long($line) === null) {
        echo "Error: invalid ip address\n";
        continue;
    }

    $sTime = yzh52521\XdbSearcher::now();
    try {
    ...
    printf("{region: %s, ioCount: %d, took: %.5f ms}\n",
        $region, $searcher->getIOCount(), yzh52521\XdbSearcher::now() - $sTime);
}
